...
public interface TimeOutDao {
    ...
    @Select("select *, CONVERT(CHAR(10),getdate(),120) as 当前日期, DATEDIFF(DAY,a.工单接收最后日期, GETDATE()) AS  超期天数\n" +
            "from\n" +
            "    (SELECT ItemNumber as 成品物料号, CONVERT(CHAR(10),max(TransactionDate),120) as 工单接收最后日期\n" +
            "     FROM [FSDBSS].[dbo].[FS_HistoryMOReceipt]   group by ItemNumber\n" +
            "    ) a\n" +
            "        LEFT JOIN\n" +
